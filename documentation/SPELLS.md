# SPELLS DOCUMENTATION

## Classification
| Tier | LIGHT | DARK | DESTRUCTIVE | NATURAL |
|------|-------|------|-------------|---------|
| 1 | [Strength](#strength) | [Weakness](#weakness) | [Magic Arrow](#magic-arrow) | [Wasp Swarm](#wasp-swarm) |
| | | | | |
| 2 | [Haste](#haste) | [Slow](#slow) | [Lightning Bolt](#lightning-bolt) | [Arcane Crystal](#arcane-crystal) |
| 2 | [Endurance](#endurance) | [Rupture](#rupture) | [Ice Bolt](#ice-bolt) | [Land Mines](#land-mines) |
| | | | | |
| 3 | [Cleansing](#cleansing) | [Decay](#decay) | [Polar Ring](#polar-ring) | [Summon Elementals](#summon-elementals) |
| 3 | [Deflect Missile](#deflect-missile) | [Sorrow](#sorrow) | [Stone spikes](#stone-spikes) | [Confusion](#confusion) |
| 3 | [Righteous Might](#righteous-might) | [Suffering](#suffering) | [Fireball](#fireball) | [Regeneration](#regeneration) |
| | | | | |
| 4 | [Divine Vengeance](#divine-vengeance) | [Teleport](#teleport) | [Fist of Wrath](#fist-of-wrath) | [Earthquake](#earthquake) |
| 4 | [Blind](#blind) | [Raise Dead](#raise-dead) | [Firewall](#firewall) | [Wasp Hive](#wasp-hive) |
| 4 | [Blade Barrier](#blade-barrier) | [Vampirism](#vampirism) | [Discharge](#discharge) | [Immunity](#immunity) |
| | | | | |
| 5 | [Celestial Shield](#celestial-shield) | [Berserk](#berserk) | [Deep Freeze](#deep-freeze) | [Meteor Shower](#meteor-shower) |
| 5 | [Resurrection](#resurrection) | [Curse of the Netherworld](#curse-of-the-netherworld) | [Implosion](#implosion) | [Mind Control](#mind-control) |
| 5 | [Holy Word](#holy-word) | [Shadow Image](#shadow-image) | [Armageddon](#armageddon) | [Conjure Phoenix](#conjure-phoenix) |
| | | | | |


## Spells data

#### Magic Arrow
>Tier 1 Destructive Magic  (5 Mana)

Shoots magic missile to deal non-phisical damage to the selected enemy creature. 

Damage :
- Base: '1'
  PerPower: '13'
- Base: '2'
  PerPower: '16'
- Base: '3'
  PerPower: '19'
- Base: '4'
  PerPower: '22'

Duration :
N/A

#### Fist of Wrath
>Tier 4 Destructive Magic  (10 Mana)

Summons magical fist to deal non-elemental physical damage to target enemy creature. This spell ignores magic resistance and protection from magic.

Damage :
- Base: '5'
  PerPower: '17'
- Base: '5'
  PerPower: '21'
- Base: '5'
  PerPower: '25'
- Base: '5'
  PerPower: '29'

Duration :
N/A

#### Lightning Bolt
>Tier 2 Destructive Magic  (7 Mana)

Deals lightning damage to selected enemy unit.

Damage :
- Base: '5'
  PerPower: '12'
- Base: '6'
  PerPower: '14'
- Base: '7'
  PerPower: '16'
- Base: '8'
  PerPower: '18'

Duration :
N/A

#### Ice Bolt
>Tier 2 Destructive Magic  (8 Mana)

Deals ice damage to selected enemy unit.

Damage :
- Base: '8'
  PerPower: '14'
- Base: '9'
  PerPower: '16'
- Base: '10'
  PerPower: '18'
- Base: '11'
  PerPower: '20'

Duration :
N/A

#### Fireball
>Tier 3 Destructive Magic  (14 Mana)

Deals fire damage to all units in the target area.

Damage :
- Base: '10'
  PerPower: '16'
- Base: '10'
  PerPower: '19'
- Base: '10'
  PerPower: '22'
- Base: '10'
  PerPower: '25'

Duration :
N/A

#### Polar Ring
>Tier 3 Destructive Magic  (15 Mana)

Deals ice damage to all units surrounding the target spot.

Damage :
- Base: '8'
  PerPower: '14'
- Base: '9'
  PerPower: '17'
- Base: '10'
  PerPower: '20'
- Base: '11'
  PerPower: '23'

Duration :
N/A

#### Discharge
>Tier 4 Destructive Magic  (24 Mana)

Deals massive lightning damage to several adjacent creatures (up to 4), starting with the selected one. Every next creature in the chain gets 75% of the damage inflicted to the previous one.  (The chain can hit the own creatures) 

Damage :
- Base: '5'
  PerPower: '16'
- Base: '6'
  PerPower: '20'
- Base: '7'
  PerPower: '24'
- Base: '8'
  PerPower: '28'

Duration :
N/A

#### Meteor Shower
>Tier 5 Natural Magic  (36 Mana)

Deals massive earth damage to all creatures in target area.

Damage :
- Base: '45'
  PerPower: '29'
- Base: '50'
  PerPower: '35'
- Base: '55'
  PerPower: '41'
- Base: '60'
  PerPower: '47'

Duration :
N/A

#### Implosion
>Tier 5 Destructive Magic  (37 Mana)

Deals earth damage to a single targeted enemy unit.

Damage :
- Base: '38'
  PerPower: '25'
- Base: '46'
  PerPower: '32'
- Base: '54'
  PerPower: '39'
- Base: '62'
  PerPower: '46'

Duration :
N/A

#### Armageddon
>Tier 5 Destructive Magic  (45 Mana)

Deals massive fire damage to all creatures and war machines on a battlefield and local physical damage in the point of impact.

Damage :
- Base: '50'
  PerPower: '16'
- Base: '60'
  PerPower: '21'
- Base: '70'
  PerPower: '26'
- Base: '80'
  PerPower: '31'

Duration :
- Base: '30'
  PerPower: '6'
- Base: '40'
  PerPower: '6'
- Base: '50'
  PerPower: '6'
- Base: '60'
  PerPower: '6'


#### Weakness
>Tier 1 Dark Magic  (4 Mana)

Causes the selected enemy unit to inflict less damage in combat. Effective on creatures that have a wide damage range.

Damage :
- Base: '0.6'
  PerPower: '-0.01'
- Base: '0.5'
  PerPower: '-0.02'
- Base: '0.4'
  PerPower: '-0.03'
- Base: '0.3'
  PerPower: '-0.04'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Slow
>Tier 2 Dark Magic  (5 Mana)

Makes target enemy stack take fewer actions in combat.

Damage :
- Base: '0.20'
  PerPower: '0.003'
- Base: '0.25'
  PerPower: '0.003'
- Base: '0.30'
  PerPower: '0.003'
- Base: '0.35'
  PerPower: '0.003'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Rupture
>Tier 2 Dark Magic  (8 Mana)

Destroys armor of target enemy stack, dealing damages and reducing its defense. Can be cast several times on a single stack, but defense will not go lower than zero.

Damage :
- Base: '5'
  PerPower: '0.15'
- Base: '6'
  PerPower: '0.20'
- Base: '7'
  PerPower: '0.25'
- Base: '8'
  PerPower: '0.30'

Duration :
N/A

#### Decay
>Tier 3 Dark Magic  (7 Mana)

Inflicts plague on target enemy stack that  receives earth damage each time it takes an action. (The spell does not work on undead, elemental and mechanical units.)

Damage :
- Base: '5'
  PerPower: '11'
- Base: '15'
  PerPower: '12'
- Base: '25'
  PerPower: '13'
- Base: '35'
  PerPower: '14'

Duration :
- Base: '2'
  PerPower: '0.04'
- Base: '2.33'
  PerPower: '0.06'
- Base: '2.66'
  PerPower: '0.08'
- Base: '3'
  PerPower: '0.10'


#### Suffering
>Tier 3 Dark Magic  (8 Mana)

Weakens the target enemy unit to decrease its Attack.

Damage :
- Base: '5'
  PerPower: '0.15'
- Base: '6'
  PerPower: '0.2'
- Base: '7'
  PerPower: '0.25'
- Base: '8'
  PerPower: '0.3'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Confusion
>Tier 3 Natural Magic  (10 Mana)

Makes creatures in target enemy stack forget what they are doing on a battlefield. Some of the affected creatures will forget to use shooting attacks and retaliation strikes. (The spell does not work on undead, elemental and mechanical units.) 

Damage :
- Base: '0.25'
  PerPower: '0.01'
- Base: '0.35'
  PerPower: '0.01'
- Base: '0.45'
  PerPower: '0.01'
- Base: '0.55'
  PerPower: '0.01'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Berserk
>Tier 5 Dark Magic  (35 Mana)

Drives target stack of creatures berserk. Affected stack considers all other creatures and war machines its personal enemies and attacks the nearest one with redoubled rage (The spell does not work on undead, elemental and mechanical units).

Damage :
- Base: '-0.50'
  PerPower: '0.01'
- Base: '-0.30'
  PerPower: '0.01'
- Base: '-0.15'
  PerPower: '0.01'
- Base: '0'
  PerPower: '0.01'

Duration :
- Base: '1'
  PerPower: '0'
- Base: '1'
  PerPower: '0'
- Base: '1'
  PerPower: '0'
- Base: '1'
  PerPower: '0'


#### Blind
>Tier 4 Light Magic  (18 Mana)

Blinds the selected enemy creature so that it cannot move, attack or use any abilities. Blindness disappears if blinded creature is attacked. Spellpower determines the duration of effect. (The spell does not work on undead, elemental and mechanical units.) 

Damage :
N/A
Duration :
- Base: '0.4'
  PerPower: '0.06'
- Base: '0.6'
  PerPower: '0.07'
- Base: '0.8'
  PerPower: '0.08'
- Base: '1'
  PerPower: '0.09'


#### Mind Control
>Tier 5 Natural Magic  (39 Mana)

Gives the hero temporary control over selected enemy unit. Unit under this spell can't counter-attack. Control will be lost if the unit is attacked (The caster should have an Expert Natural Magic skill to gain the control over tier 7 creatures. The spell does not work on undead, elemental and mechanical units.) 

Damage :
- Base: '0.5'
  PerPower: '0.01'
- Base: '0.6'
  PerPower: '0.01'
- Base: '0.7'
  PerPower: '0.01'
- Base: '0.8'
  PerPower: '0.01'

Duration :
- Base: '1.5'
  PerPower: '0.005'
- Base: '1.5'
  PerPower: '0.01'
- Base: '1.5'
  PerPower: '0.02'
- Base: '1.5'
  PerPower: '0.03'


#### Curse of the Netherworld
>Tier 5 Dark Magic  (40 Mana)

Deals unholy damage to all creatures on a battlefield except undead and infernal ones.

Damage :
- Base: '100'
  PerPower: '10'
- Base: '135'
  PerPower: '13'
- Base: '165'
  PerPower: '16'
- Base: '200'
  PerPower: '20'

Duration :
N/A

#### Strength
>Tier 1 Light Magic  (4 Mana)

Causes the selected friendly unit to inflict more damage in combat. Effective on creatures that have a wide damage range.

Damage :
- Base: '0.4'
  PerPower: '0.01'
- Base: '0.5'
  PerPower: '0.02'
- Base: '0.6'
  PerPower: '0.03'
- Base: '0.7'
  PerPower: '0.04'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Haste
>Tier 2 Light Magic  (5 Mana)

Causes the selected friendly unit to act more frequently in combat.

Damage :
- Base: '0.20'
  PerPower: '0.003'
- Base: '0.25'
  PerPower: '0.003'
- Base: '0.30'
  PerPower: '0.003'
- Base: '0.35'
  PerPower: '0.003'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Endurance
>Tier 2 Light Magic  (6 Mana)

Increases the selected friendly unit's defense strength. Spellpower determines the duration of effect.

Damage :
- Base: '5'
  PerPower: '0.15'
- Base: '6'
  PerPower: '0.20'
- Base: '7'
  PerPower: '0.25'
- Base: '8'
  PerPower: '0.30'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Cleansing
>Tier 3 Light Magic  (12 Mana)

Dispels positive magic effects from enemy creatures and negative magic effects from friendly creatures. Checks against caster level to dispel effects, so low level caster would be unlikely to dispel magic cast by high level mage.

Damage :
- Base: '0.1'
  PerPower: '0.03'
- Base: '0.3'
  PerPower: '0.03'
- Base: '0.5'
  PerPower: '0.03'
- Base: '0.7'
  PerPower: '0.03'

Duration :
N/A

#### Righteous Might
>Tier 3 Light Magic  (8 Mana)

Affected creature is filled with rage and a lust for blood, it gains bonus to attack.

Damage :
- Base: '5'
  PerPower: '0.15'
- Base: '6'
  PerPower: '0.20'
- Base: '7'
  PerPower: '0.25'
- Base: '8'
  PerPower: '0.30'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Deflect Missile
>Tier 3 Light Magic  (10 Mana)

Makes the selected friendly unit receive less damage from ranged attacks.

Damage :
- Base: '0.30'
  PerPower: '0.005'
- Base: '0.35'
  PerPower: '0.006'
- Base: '0.40'
  PerPower: '0.007'
- Base: '0.45'
  PerPower: '0.008'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Immunity
>Tier 4 Natural Magic  (13 Mana)

Clears both positive and negative magic effects from the target stack and makes it immune to further magic spell effects. Can only be cast on friendly stack.

Damage :
- Base: '2'
  PerPower: '0'
- Base: '3'
  PerPower: '0'
- Base: '4'
  PerPower: '0'
- Base: '5'
  PerPower: '0'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Teleport
>Tier 4 Dark Magic  (16 Mana)

Teleports targeted friendly creature to another location on a battlefield (Only expert in Dark Magic can teleport creatures over town walls)

Damage :
N/A
Duration :
N/A

#### Celestial Shield
>Tier 5 Light Magic  (38 Mana)

Encloses target stack with magical energy shield that partly drains all damage dealt to the stack.

Damage :
- Base: '0.25'
  PerPower: '0.005'
- Base: '0.33'
  PerPower: '0.005'
- Base: '0.50'
  PerPower: '0.005'
- Base: '0.66'
  PerPower: '0.005'

Duration :
- Base: '90'
  PerPower: '30'
- Base: '110'
  PerPower: '45'
- Base: '130'
  PerPower: '60'
- Base: '150'
  PerPower: '75'


#### Holy Word
>Tier 5 Light Magic  (40 Mana)

Deals holy damage to all the undead and infernal creatures on a battlefield.

Damage :
- Base: '100'
  PerPower: '10'
- Base: '135'
  PerPower: '13'
- Base: '165'
  PerPower: '16'
- Base: '200'
  PerPower: '20'

Duration :
N/A

#### Land Mines
>Tier 2 Natural Magic  (7 Mana)

Puts randomly several magical explosive mines in a selected area of the battlefield. Mines are invisible to the enemy.

Damage :
- Base: '10'
  PerPower: '14'
- Base: '16'
  PerPower: '16'
- Base: '22'
  PerPower: '18'
- Base: '28'
  PerPower: '20'

Duration :
- Base: '5'
  PerPower: '0.10'
- Base: '5.5'
  PerPower: '0.14'
- Base: '6'
  PerPower: '0.18'
- Base: '6.5'
  PerPower: '0.22'


#### Wasp Swarm
>Tier 1 Natural Magic  (6 Mana)

Summons a swarm of insects to inflict magical damage on the selected enemy creature. (The spell does not work on undead, elemental and mechanical units.)

Damage :
- Base: '3'
  PerPower: '9'
- Base: '5'
  PerPower: '11'
- Base: '7'
  PerPower: '13'
- Base: '9'
  PerPower: '15'

Duration :
- Base: '0'
  PerPower: '0.01'
- Base: '0.13'
  PerPower: '0.01'
- Base: '0.26'
  PerPower: '0.01'
- Base: '0.39'
  PerPower: '0.01'


#### Shadow Image
>Tier 5 Dark Magic  (22 Mana)

Copies the selected friendly unit (only one copy can be made). The copy can't cast any spells or use any abilities and disappears as soon as it recieves any damage (Incorporial ability gives the copy 50% chance to avoid any damage)

Damage :
- Base: '2'
  PerPower: '0.1'
- Base: '3'
  PerPower: '0.1'
- Base: '4'
  PerPower: '0.1'
- Base: '5'
  PerPower: '0.1'

Duration :
N/A

#### Earthquake
>Tier 4 Natural Magic  (25 Mana)

Earthquake damages and stuns all creatures. Also affects town walls during a siege.

Damage :
- Base: '110'
  PerPower: '10'
- Base: '140'
  PerPower: '15'
- Base: '170'
  PerPower: '20'
- Base: '200'
  PerPower: '25'

Duration :
- Base: '55'
  PerPower: '6'
- Base: '70'
  PerPower: '9'
- Base: '85'
  PerPower: '12'
- Base: '100'
  PerPower: '15'


#### Raise Dead
>Tier 4 Dark Magic  (15 Mana)

Reanimates creatures in target friendly stack. Undead creatures are brought back to "life", all other creatures are re-animated for the duration of the battle only. Every use of this spell decreases hit points of target stack.

Damage :
- Base: '15'
  PerPower: '19'
- Base: '15'
  PerPower: '28'
- Base: '15'
  PerPower: '37'
- Base: '15'
  PerPower: '46'

Duration :
- Base: '0.24'
  PerPower: '-0.004'
- Base: '0.24'
  PerPower: '-0.004'
- Base: '0.24'
  PerPower: '-0.004'
- Base: '0.24'
  PerPower: '-0.004'


#### Summon Elementals
>Tier 3 Natural Magic  (20 Mana)

Summons fire, earth, water or air elementals (depending on the battlefield terrain) to fight for the hero's cause.

Damage :
- Base: '1'
  PerPower: '1.6'
- Base: '5'
  PerPower: '2.0'
- Base: '10'
  PerPower: '2.4'
- Base: '15'
  PerPower: '2.8'

Duration :
N/A

#### Resurrection
>Tier 5 Light Magic  (30 Mana)

Permanently resurrects creatures in target friendly stack.


Damage :
- Base: '10'
  PerPower: '24'
- Base: '20'
  PerPower: '33'
- Base: '30'
  PerPower: '42'
- Base: '40'
  PerPower: '51'

Duration :
- Base: '0'
  PerPower: '0'
- Base: '0'
  PerPower: '0'
- Base: '0'
  PerPower: '0'
- Base: '0'
  PerPower: '0'


#### Conjure Phoenix
>Tier 5 Natural Magic  (42 Mana)

Summons a phoenix on the battle field to fight on the hero’s side. Only one phoenix can be summoned at a time

Damage :
- Base: '7'
  PerPower: '0'
- Base: '6'
  PerPower: '0'
- Base: '5'
  PerPower: '0'
- Base: '4'
  PerPower: '0'

Duration :
N/A

#### Firewall
>Tier 4 Destructive Magic  (22 Mana)

Creates a wall of fire in the selected area on battlefield. Deals fire damage to all creatures in this area during several turns.

Damage :
- Base: '20'
  PerPower: '18'
- Base: '25'
  PerPower: '22'
- Base: '30'
  PerPower: '26'
- Base: '35'
  PerPower: '30'

Duration :
- Base: '1.5'
  PerPower: '0.06'
- Base: '2'
  PerPower: '0.07'
- Base: '2.5'
  PerPower: '0.08'
- Base: '3'
  PerPower: '0.09'


#### Stone spikes
>Tier 3 Destructive Magic  (11 Mana)

Deals earth damage to all creatures in target area (cross form)

Damage :
- Base: '6'
  PerPower: '15'
- Base: '8'
  PerPower: '19'
- Base: '10'
  PerPower: '23'
- Base: '12'
  PerPower: '27'

Duration :
N/A

#### Sorrow
>Tier 3 Dark Magic  (13 Mana)

Decreases morale and luck of selected enemy creature

Damage :
- Base: '1'
  PerPower: '0.02'
- Base: '2'
  PerPower: '0.02'
- Base: '3'
  PerPower: '0.02'
- Base: '4'
  PerPower: '0.02'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Vampirism
>Tier 4 Dark Magic  (25 Mana)

Selected friendly creature gets all Undead attributes and the ability to drain life just like vampires.

Damage :
- Base: '20'
  PerPower: '0.5'
- Base: '25'
  PerPower: '0.5'
- Base: '30'
  PerPower: '0.5'
- Base: '35'
  PerPower: '0.5'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Deep Freeze
>Tier 5 Destructive Magic  (26 Mana)

Freezes the target, immediately inflicting ice damage and causing it to suffer increased physical damage during a certain amount of time due to frostbites.


Damage :
- Base: '20'
  PerPower: '20'
- Base: '24'
  PerPower: '24'
- Base: '28'
  PerPower: '28'
- Base: '32'
  PerPower: '32'

Duration :
- Base: '1'
  PerPower: '0'
- Base: '1'
  PerPower: '0'
- Base: '1'
  PerPower: '0'
- Base: '1'
  PerPower: '0'


#### Regeneration
>Tier 3 Natural Magic  (11 Mana)

The target creature gains health regeneration on its turn. Does not work on Undead, Mechanical and Elemental creatures.

Damage :
- Base: '40'
  PerPower: '2'
- Base: '50'
  PerPower: '3'
- Base: '60'
  PerPower: '4'
- Base: '70'
  PerPower: '5'

Duration :
- Base: '1.5'
  PerPower: '0.05'
- Base: '1.5'
  PerPower: '0.1'
- Base: '1.5'
  PerPower: '0.2'
- Base: '1.5'
  PerPower: '0.3'


#### Divine Vengeance
>Tier 4 Light Magic  (20 Mana)

Inflicts magical damage to selected creature that depends on the number of enemies selected creature killed from the beginning of combat.

Damage :
- Base: '21'
  PerPower: '1.1'
- Base: '24'
  PerPower: '1.4'
- Base: '27'
  PerPower: '1.7'
- Base: '30'
  PerPower: '2.0'

Duration :
N/A

#### Arcane Crystal
>Tier 2 Natural Magic  (9 Mana)

Creates a crystal on the selected place on the field, blocking the way until the end of combat or until the crystal is attacked and destroyed.

Damage :
- Base: '22'
  PerPower: '14'
- Base: '30'
  PerPower: '18'
- Base: '38'
  PerPower: '22'
- Base: '46'
  PerPower: '26'

Duration :
- Base: '1'
  PerPower: '0.1'
- Base: '1'
  PerPower: '0.1'
- Base: '1'
  PerPower: '0.1'
- Base: '1'
  PerPower: '0.1'


#### Wasp Hive
>Tier 4 Natural Magic  (16 Mana)

Creates a wasp hive on the selected place of the field. A swarm of wasps from this hive will attack enemy creatures until the end of combat or until the hive is attacked and destroyed (The hive will not attack undead, elemental and mechanical units).

Damage :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'

Duration :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'


#### Blade Barrier
>Tier 4 Light Magic  (14 Mana)

Temporarily creates a wall, three squares long, on the selected part of the field. The wall has additional 75% Defense from the Range and Magical Attacks and returns damage equal to the number of lost Hit Points to the melee attackers.

Damage :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'

Duration :
- Base: '1'
  PerPower: '0.1'
- Base: '1'
  PerPower: '0.1'
- Base: '1'
  PerPower: '0.1'
- Base: '1'
  PerPower: '0.1'


#### Ralling Cry
>Tier 1 Warcries  (7 Mana)

Increases morale of all creatures in army for two turns depending on the hero's level.
Barbarian heroes also gives additional rage points to orcs in their army.

Damage :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'

Duration :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'


#### Call of Blood
>Tier 2 Warcries  (8 Mana)

Selected friendly Raging Blooded creature gets additional number of rage points depending on hero's level. (Barbarians only)

Damage :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'

Duration :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'


#### Word of the Chief
>Tier 1 Warcries  (0 Mana)

Hero attacks the selected friendly creature (dealing his standard damage). This attack hastens the creature's turn, depending on the hero's level. This warcry does not work on undead, elemental and mechanical units.
Barbarian heroes also gives additional rage points to orcs in their army.

Damage :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'

Duration :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'


#### Fear My Roar
>Tier 2 Warcries  (12 Mana)

Hero has a chance to scare selected enemy creature, depending on the ratio between the hero's and the creature's levels. This warcry does not work on creatures immune to mind control effects.
Barbarian heroes also gives additional rage points to orcs in their army.

Damage :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'

Duration :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'


#### Battlecry
>Tier 3 Warcries  (15 Mana)

Increases the Attack of all creatures in hero's army by +5 and their Speed by +1. Barbarian heroes also gives additional rage points to orcs in their army.

Damage :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'

Duration :
- Base: '2'
  PerPower: '1'
- Base: '2'
  PerPower: '1'
- Base: '2'
  PerPower: '1'
- Base: '2'
  PerPower: '1'


#### Horde’s Anger
>Tier 3 Warcries  (18 Mana)

Selected enemy receives magical damage equal to half the number of creatures in the hero's army.
Barbarian heroes also gives additional rage points to orcs in their army.

Damage :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'

Duration :
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'
- Base: '1'
  PerPower: '1'

